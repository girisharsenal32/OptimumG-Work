...
namespace Coding_Attempt_with_GUI
{
    ...
    public class KO_Optimizer_ActuationPoints : KO_Master_Optimizer, IOptimizer_KO
    {
        ...
        /// <summary>
        /// <see cref="List{Double}"/> holding the Motion Ratio Values as they vary with the <see cref="SpringDeflection_Deltas"/> and <see cref="Calc_WheelDeflection_Deltas"/>
        /// </summary>
        ...
        public double EvaluateFitnessCurve(Chromosome chromosome)
        {
            ...
            if (chromosome != null)
            {
                ...
                PushrodRockerPoint_ForMotionAnalysis = new Point3D(x, y, 0);
        ...
        public bool TerminateAlgorithm(Population _population, int _currGeneration, long currEvaluation)
        {
            if (_currGeneration > No_Generations || _population.MaximumFitness>=0.99)
            {
                return true;
            }
            else
        ...
        private void ExtractChromosoomes(Chromosome chromosome, out Angle _damperRotAngle, out double _x, out double _y)
        {
            ...
            double upperAngle = 70/*360*/;
            double lowerAngle = -70/*0*/;

            ///<summary>
            ///Assigning the Nominal,Upper and Lower Values of the X/Y/Z Coordinates. This will be later on obtained from the User
            /// </summary>
            double nominalX = KO_CV.VCornerParams.Rocker_Center.X;
            ///<remarks>
            ///Upper and Lower limits of the X Coordinate.
            ///Arbitrary as of no
            /// </remarks>
            double upperDeltaX = 65;
            double lowerDeltaX = /*-65*/0;

            double nominalY = KO_CV.VCornerParams.Rocker_Center.Y;
            ///<remarks>
            ///Upper and Lower limits of the X Coordinate.
            ///Arbitrary as of no
            /// </remarks>
            double upperDeltaY = 65;
            double lowerDeltaY = /*-65*/0;
        ...
        private void SolveKinematics_MotionRatioEval(double _springDeflection, Vehicle _vehicle, int _index, DoubleWishboneKinematicsSolver _dwSolver)
        {
            ...
            Rotate_Rocker(rockerRotationAngle, KO_CV.VCornerParams.Rocker_Axis_Vector, KO_CV.VCornerParams.Rocker_Center, ref PushrodRockerPoint_ForMotionAnalysis, ref DamperRockerPoint_ForMotionAnalysis);
            ...
            _dwSolver.KO_InitialStage_PushPullOutboard(new MathNet.Spatial.Euclidean.Point3D(PushrodRockerPoint_ForMotionAnalysis.X, PushrodRockerPoint_ForMotionAnalysis.Y, PushrodRockerPoint_ForMotionAnalysis.Z), new MathNet.Spatial.Euclidean.Point3D(), out MathNet.Spatial.Euclidean.Point3D G);
        ...
        private void Rotate_Rocker(Angle _rotAngle, Vector3D _axisRotation, Point3D _centerRot, ref Point3D _newPushPullRocker, ref Point3D _newDamperRocker)
        {
        ...
        private void Extract_MotionRatio()
        {
            ...
            for (int i = 0; i < KO_CV.BumpSteerCurve.WheelDeflections.Count; i++)
            {
                if (i != 0 && KO_CV.BumpSteerCurve.WheelDeflections[i] == 1)
                {
                    ...
                    {
                        MotionRatio.Add(Calc_WheelDeflection_Deltas[i] / SpringDeflection_Deltas[i]);
                    }
                    else if (KO_CV.MotionRatio_Format == MotionRatioFormat.ShockByWheel)
                    {
                        MotionRatio.Add(SpringDeflection_Deltas[i] / Calc_WheelDeflection_Deltas[i]);
                    }
